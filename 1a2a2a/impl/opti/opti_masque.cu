	...
	uint * masque, uint POIDS, float p)
{
	...
	if (thx < POIDS) {
		masque[thx] = (PSEUDO_ALEA(graine+thx) >= p ? NON_MASQUEE : MASQUEE);
		//	0 - pas masque
		//	1 - masque
	...
	Mdl_t * mdl, float * p)
{
	...
	masque[0] = cudalloc<uint>(mdl->ST[0]);
	kerd_cree_masque<<<dim3(KERD(mdl->ST[0],1024)), dim3(1024)>>>(
		rand() % 10000,
		masque[0], mdl->ST[0], p[0]
	);
	ATTENDRE_CUDA();
	//
	FOR (1, c, C) {
...
static void __liberer_masque(uint ** masque) {
	FOR(0, c, C) cudafree<uint>(masque[c]);
	free(masque);
}
...
Masque_t * cree_masque(Mdl_t * mdl, float * p) {
	...
	//
	ret->masque = __cree_masque(mdl, p);
...
void sortire_masque(Mdl_t * mdl, Masque_t * masque) {
	...
	__liberer_masque(masque->masque);
	free(masque);
};